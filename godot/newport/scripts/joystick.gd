	...
	if event is InputEventScreenTouch:
		...
		if event.index == active_joystick_index:
			...
			if not active_camera_swipe_jump_triggered and _can_swipe_jump():
				...
				var swipe_up_distance := -total_delta.y
				var horizontal_drift :float = abs(total_delta.x)
				if swipe_up_distance >= swipe_jump_threshold and horizontal_drift <= swipe_jump_horizontal_tolerance:
...
func _start_joystick_touch(touch_pos: Vector2, touch_index: int, touch_joystick: Node):
	...
	lock_candidate_started_at = -1.0
	last_drag_was_north = false
	# Position joystick base at the touch location.
...
func _queue_jump_press(duration: float = jump_press_duration) -> void:
	...
	var release_token := jump_release_token
	_press_key("jump")
	await get_tree().create_timer(duration).timeout
	if release_token == jump_release_token:
		_release_key("jump")
...
func _control_tree_contains_point(root: Control, screen_pos: Vector2) -> bool:
	var local_pos := root.get_global_transform_with_canvas().affine_inverse() * screen_pos
	if Rect2(Vector2.ZERO, root.size).has_point(local_pos):
		return true
	for child in root.get_children():
		var child_control := child as Control
		if child_control == null or not child_control.is_visible_in_tree():
			continue
		var child_local_pos := child_control.get_global_transform_with_canvas().affine_inverse() * screen_pos
		if Rect2(Vector2.ZERO, child_control.size).has_point(child_local_pos):
			return true
