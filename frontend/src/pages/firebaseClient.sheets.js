import { db, get, ref, set, update, remove } from "../lib/rtdbSheets";
import { trackUserJoined, trackUserRegistered } from "../lib/analyticsClient";

// Sheets-backed drop-in for pages/firebaseClient.js. Same exports and behavior,
// backed by the Apps Script web app (see /appsscript). Configure via
// VITE_SHEETS_API_URL / VITE_SHEETS_ACCESS_TOKEN in frontend/.env.
...
export async function fetchUserRoles(username) {
  ...
  try {
    ...
    console.error(`[Sheets] fetchUserRoles failed for ${username}, falling back to defaults:`, error);
