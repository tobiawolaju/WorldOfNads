...
export default function AdminAnalytics() {
  ...
  const loadAnalytics = async () => {
    ...
    try {
      ...
      setAccessError("Failed to load analytics data.");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    const stored = sessionStorage.getItem("analytics_access");
    if (stored === "granted") {
  ...
  const verifyAccess = async () => {
    ...
    try {
      ...
      sessionStorage.setItem("analytics_access", "granted");
      setAccessGranted(true);
      await loadAnalytics();
    } catch (error) {
  ...
  const handleExport = async (format: "csv" | "json") => {
    try {
      ...
      setAccessError("Export failed. Try again.");
    }
  };
  ...
  if (!accessGranted) {
    return (
      <div className="analytics-auth">
        <div className="analytics-auth__card">
          <h1>Analytics Access</h1>
          <p>Enter your admin access code to view metrics.</p>
          <input
            type="password"
            value={accessCode}
