...
export default function SpounsorDashbaord() {
  ...
  const handleCreateMatch = async () => {
    if (!form.sponsor || !form.prizeAmount || !form.date || !form.time) {
      showErrorToast("Fill sponsor, prize amount, match date, and match time.");
      return;
    }
    ...
    if (!Number.isFinite(minPlayersValue) || minPlayersValue < 1) {
      showErrorToast("Minimum players must be at least 1.");
      return;
    }

    if (maxPlayersValue !== null && (!Number.isFinite(maxPlayersValue) || maxPlayersValue < minPlayersValue)) {
      showErrorToast("Maximum players must be greater than or equal to minimum players.");
      return;
    }

    const conflict = findConflictingMatch(form.date, form.time, allMatches);
    if (conflict) {
      showErrorToast("That time is already booked or too close to another match. Please select another time.");
      return;
    }

    if (!activeEthWallet) {
      showErrorToast("No Ethereum wallet found. Please ensure your Privy wallet is created and ready.");
      return;
    }

    const minStartTime = new Date(Date.now() + 5 * 60 * 1000); // at least 5 minutes from now
    if (new Date(`${form.date}T${form.time}`) < minStartTime) {
      showErrorToast("Match start time must be at least 5 minutes from now. Please pick a later time.");
      return;
    }

    setIsSubmitting(true);
    setIsModalOpen(false);
    showSuccessToast("Creating match, please approve the transaction in your wallet...");
    ...
    try {
      ...
      showSuccessToast(
        contractResult.mode === "onchain"
          ...
          : "Match created with mock contract confirmation."
      );
    } catch (error) {
      console.error(error);
  ...
  const handleSuggestTime = () => {
    ...
    if (!suggestion) {
      showErrorToast("No available time found in the next 60 days. Try another date.");
      return;
    }
    ...
    showSuccessToast(`Suggested time: ${suggestion.date} at ${suggestion.time}`);
  };
