...
@onready var anim_run: AnimationPlayer = $running     # 👈 Running animation player
@onready var anim_idle: AnimationPlayer = $idle       # 👈 Idle animation player
@onready var mesh: Skeleton3D = $Skeleton3D   # 👈 Visible character mesh
...
func _ready() -> void:
	...
	# Ensure idle animation starts by default
	_play_idle()
...
# Player.gd (only the _physics_process function is shown, the rest is the same)
...
func _physics_process(delta: float) -> void:
	...
	if inputs.forward: input_dir.y += 1
	if inputs.back: input_dir.y -= 1
	if inputs.left: input_dir.x -= 1
	if inputs.right: input_dir.x += 1
	input_dir = input_dir.normalized()

	# === Apply gravity & jump ===
	# --- FIX #2: IMPROVED GROUNDED LOGIC ---
	if not is_on_floor():
		velocity_y -= GRAVITY * delta
	else: # If we are on the floor
		velocity_y = 0 # Reset vertical velocity to prevent accumulating downward force
		if inputs.jump:
	...
	# Keep CharacterBody3D aligned with camera rotation
...
func _update_camera(delta: float) -> void:
	...
	if result and result.has("position"):
		desired_cam_pos = result.position + result.normal * 0.1
